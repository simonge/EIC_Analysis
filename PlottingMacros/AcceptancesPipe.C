...
std::vector<TString> fileNames = {"/scratch/EIC/Analysis/tempJustPipe.root"};


void AcceptancesPipe(){
  ...
  auto acceptedPhiRes  = df.Filter("iFilter&&(Tag1_4||Tag2_4)&&(TMath::Pi()-scatteredElectron.Theta())>0.001").Histo2D({"EQacceptedPhi", "Phi Resolvable;Electron Energy [GeV];Log(Q^{2})", eBins, eMin, eMax, qBins, qMin, qMax }, "eE","logQ2");
